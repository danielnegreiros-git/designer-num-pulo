...
- **`git add -A` só quando a árvore for sua.** Pode haver outra sessão trabalhando no mesmo repo ao mesmo tempo — o Daniel roda mais de uma. `add -A` captura o trabalho dela pela metade e congela um estado intermediário no histórico. Antes de commitar, `git status`: se aparecer arquivo de peça que você não tocou, **adicione por caminho explícito** só o que é seu. Pago em 2026-08-08: commitei a peça de thumbnail de outra sessão no meio da troca de foto dela. Não quebra (git é aditivo, ela commita por cima), mas suja o histórico e não é meu para commitar.
